...
<div class="container">
    <form method="post" action="{{ route('postagem.update', ['id' => $postagem->id]) }}" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" value="{{ $postagem->titulo }}" name="titulo" id="titulo" class="form-control" placeholder="Título da postagem" required>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <img src="{{ URL::asset('storage') }}/{{ $img->imagem }}" class="img-responsive" style="max-height:100px; max-width:100px;">
            @endforeach
            @endif
    ...
    @if (count($postagem->imagens) > 0)
    @foreach ($postagem->imagens as $img)
    <form id="deletar-imagens{{ $img->id }}" action="{{ route('postagem.delete_imagem', ['id' => $img->id]) }}" method="post">
        @csrf
        @method('delete')
    </form>
    @endforeach
    ...
    @if (count($postagem->arquivos) > 0)
    @foreach ($postagem->arquivos as $arquivo)
    <form id="deletar-arquivos{{ $arquivo->id }}" action="{{ route('postagem.delete_arquivo', ['id' => $arquivo->id]) }}" method="post">
        @csrf
        @method('delete')
    </form>
    @endforeach
